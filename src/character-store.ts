...
export interface CharacterStore {
  ...
  readonly updateCharacterName: (id: number, name: string) => void
  readonly close: () => void
}
...
export const openCharacterStore = (dataDir: string): CharacterStore => {
  ...
  const updateCharacterNameStmt = db.prepare(
    `UPDATE character_account SET character_name = ? WHERE character_id = ?`,
  )
  ...
  return {
    ...
    updateCharacterName: (id, name) => {
      updateCharacterNameStmt.run(name, id)
    },
    close: () => db.close(),
  }
}
